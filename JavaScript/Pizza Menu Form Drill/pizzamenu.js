...
//Radio and Checkbox inputs are from 'Dynamic Web Coding' Tutorial.

/* Radio button selctions are assesed by using the HTML groups input value 
and pulling from an array object that comprises selection.*/ 

/* The corresponding numerical values for the selection are obtained from the HTML
input element.*/
...
function getCheckboxPrice (classname) { 
	var checked = getCheckboxVal(classname);
		var checkCount = checked.length;//Price of a topping is conveniently 1, the same unit for array enumeration.
			if (checkCount > 1) {
			checkCount = (checkCount - 1);//First topping is complementary.
			} else {
			checkCount = 0;
			}
...
// modified version of Danny Goodman's (JS Bible) from Dynamic Web Coding.
